...
class SingleEliminationMatchDetectorTest {
  ...
  void testDetectMatchesUpperBracket() {
    ...
    var m2 = new Match(group.getOpponent(2), group.getOpponent(3));
    ...
    var pendingMatches = semd.getPendingMatches(group);
  ...
  void testGetLosersOnLevel() {
    ...
    // Matches auf Level-1
    var match1 = new Match(group.getOpponent(0), group.getOpponent(2));
    match1.addResult(new Result(0, 0, 1));
    match1.setPosition(1);
    group.getMatches().add(match1);

    // Verlierer auf Level 0 sind: 1, 3, 4, null
    var losersOnLevel0 = semd.getUpperBracketLosersOnLevel(0, group);

    assertThat(losersOnLevel0).hasSize(4);
    assertThat(((Player) losersOnLevel0.getFirst()).getPerson().getFirstName()).isEqualTo("1");
    ...
    var losersOnLevel1 = semd.getUpperBracketLosersOnLevel(1, group);
    assertThat(losersOnLevel1).hasSize(2);
    assertThat(((Player) losersOnLevel1.getFirst()).getPerson().getFirstName()).isEqualTo("0");
    assertThat(losersOnLevel1.get(1)).isNull();

    // Verlierer auf Level 2 sind: null
    var losersOnLevel2 = semd.getUpperBracketLosersOnLevel(2, group);
    assertThat(losersOnLevel2).hasSize(1);
    assertThat(losersOnLevel2.getFirst()).isNull();
  }
